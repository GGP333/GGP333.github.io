...
My focus is on building **AI solutions with the potential for large, real-world impact in healthcare** — particularly in radiology and oncology — by designing systems that are explainable, trustworthy, and clinically usable. I work at the **bridge between academia and industry**: my goal is to take rigorous academic research in deep learning and multimodal medical imaging and turn it into tools that are actually deployed inside **real healthcare systems** — hospitals, clinical workflows, and medical-device pipelines. In practice this means combining solid methodology (multimodal segmentation, vision–language models, LLM-based assistants) with the integration, validation, and interoperability work needed to put those models in front of clinicians and have them be useful day to day. The long-term goal is transferable, explainable diagnostic support that strengthens clinical decision-making, especially in resource-limited regional hospitals, beyond the Metropolitan Region of Chile.
...
- **VIZCACHA** (VIU25P0233 — *Visualization in Cancer with Assisted Chat*): conversational radiology platform built on **LLMs + RAG**, with a companion module **ClinParse** for medical OCR / clinical NLP developed with the [Speaknosis](https://i-health.cl/) startup.
